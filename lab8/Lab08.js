...
var current_on = document.querySelector(".on");
let index = document.querySelector("div.buttons").children;
var picnum = 0;
...
function move_right(){
    ...
    for (var i = 0, q = span.length; i<q; i++) {
        span[i].className = "";
    }
    span[picnum].className = "on";
    var interval;
    if (wrap.style.left==='0px'){
        interval = -2400;
    }else {
        interval= parseInt(wrap.style.left)+600;
    }
    wrap.style.left = interval + 'px';
}
...
arrow_right.addEventListener("click",move_right);
arrow_left.addEventListener("click",move_left);
...
container.addEventListener("mouseout", function () {
    clock = setInterval(move_right, 2000);
});
...
for (let i = 0; i < 5; i++) {
    span[i].addEventListener("click", function () {
        current_on.className = "";
        span[i].className = "on";
        picnum = i + 1;
        current_on = index[i];
        wrap.style.left = (-picnum * 600) + "px";
    },false)
}
/*********************************************end*************************************/
